...
family_tax_benefit <- function(.data, 
  ...
  if(or(!all(lapply(.data, class)[c('id_hh','id', 'maintenance_income', 'maintenance_children')] 
        ...
        !all(lapply(.data, class)[c('in_secondary_school', 'single_parent', 'other_allowance_benefit_or_pension')] 
             == 'logical'))){
    stop("`.data` columns are not of correct class. Only integer, numeric, or logical allowed.")
  }
  ...
  family_data <- .data %>% group_by(id_hh) %>% summarise(family_income = sum(income), 
