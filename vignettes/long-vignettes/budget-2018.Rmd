...
if (FALSE) {
  if (interactive() && !is_knitting()) {
    ...
    change_pngs <- function(html) {
      ...
      for (i in grep("((Budget2018.atlas)|(budget.2018_files)).*\\.png", html_lines)) {
...
get_near_term <- function(fy_year = c("2016-17", "2017-18", "2018-19")) {
  ...
  if (cache && file.exists(syyyy.fst <- sprintf("budget-2018_cache/%s.fst", syyyy))) {
...
```{r project_useTreasurys, cache=TRUE}
.project_useTreasurys <- lapply(1:13, .project_to, use.Treasury = TRUE)
```

```{r .project_useGrattans, cache=TRUE}
...
model_Budgets <- function(fy_year, 
  ...
  list(
    ...
    Budget2018_medicare25 = model_income_tax(medicare_levy_rate = 0.015,
                                             ordinary_tax_rates = ordinary_tax_rates(fy_year),
    ...
    Budget2018_just_rates = model_income_tax(ordinary_tax_rates =  ordinary_tax_rates(fy_year),
                                             ...
                                             lito_202223 = FALSE,
                                             lamington = FALSE),
    Budget2018_lamington_plus_LITO = model_income_tax(lito_202223 = fy_year >= "2022-23",
                                                      lamington = fy_year %in% c("2018-19", "2019-20", "2020-21", "2021-22")),
...
if (file.exists(bound_models.fst <- "budget-2018_cache/bound_models.fst")) {
  bound_models <- read_fst(bound_models.fst, as.data.table = TRUE)
} else {
  
  bind_Budget2018_models <- function() {
  ...
  bound_models <- bind_Budget2018_models()
  
  fst::write_fst(bound_models, bound_models.fst)
...
```{r Budget-summaries}
bound_models[, .(x = revenue_foregone(.SD)), keyby = c("fy_year", "id")] %>%
  .[, x := round(x / 1e9, 2)] %>%
  dcast(id ~ fy_year, value.var = "x") %>%
  kable
```
...
```{r revenue_over_10_kable}
cost_by_policy <- 
  bound_models %>%
  .[, .(x = revenue_foregone(.SD)),
    keyby = c("fy_year", "id")] %>%
  .[, .(x = sum(x)), keyby = "id"] %>%
  .[, x := as.integer(x / 1e9)] %>%
  setnames("x", "Revenue over 10 years")
cost_by_policy %T>%
  fwrite("Budget201718-summaries-2.csv") %>%
  kable
```
...
```{r}
Brackets <- 
  bound_models[.(unique(fy_year), "Budget2018"), 
...
avg_individual_impact_by_Bracket_fy <-
  Brackets[setkey(bound_models[.(unique(fy_year), "Budget2018"),
...
avg_tax_rates_by_percentile_facet_fy <- 
{
  rbindlist(
    list(
      ...
      "Baseline" = {
        bound_models[.(unique(fy_year), "Budget2018_baseline"),
          .(avg_tax_rate = mean(coalesce(new_tax / Taxable_Income, 0)),
            avg_tax_bill = mean(new_tax),
            total_tax = sum(new_tax * WEIGHT / 1e9),
            min_income = min(Taxable_Income),
...
ShareTaxPaid_by_Decile_id <- 
  list("2017-18" = s1718[, .(Taxable_Income, tax)],
       "2027-28 baseline" = bound_models_wage2.75[.("2027-28", "Budget2018_baseline"), .(Taxable_Income, tax = new_tax)],
       "2027-28 budget" = bound_models_wage2.75[.("2027-28", "Budget2018"), .(Taxable_Income, tax = new_tax)]
  ) %>%
  rbindlist(use.names = TRUE,
            fill = TRUE,
            idcol = "id") %>%
...
print(revenue_foregone(bound_models[.("2018-19", "Budget2018_baseline_brackets_cpi")]))
```

```{r}
avg_tax_rate_by_percentile_brackets_cpi <-
  bound_models[.(unique(fy_year), "Budget2018_baseline_brackets_cpi"),
...
s2728_various_wages <- 
  ...
  grattan::model_income_tax(
      ...
      .[, .(Ind, age_range, Total_PP_BE_amt, Total_NPP_BE_amt, Taxable_Income, new_tax, baseline_tax, WEIGHT, id = wage_r)] 
  }) %>%
...
s2425_Deloitte_LF_growth <- 
  ...
  model_income_tax(baseline_fy = "2017-18", 
                   ...
                   Budget2018_lito_202223 = TRUE,
                   medicare_levy_lower_threshold = medicare_levy_lower_threshold("2024-25"), 
                   medicare_levy_upper_threshold = 31175,
                   medicare_levy_lower_sapto_threshold = medicare_levy_lower_sapto_threshold("2024-25"), 
                   medicare_levy_upper_sapto_threshold = 49300,
                   medicare_levy_lower_family_threshold = medicare_levy_lower_family_threshold("2024-25"), 
                   medicare_levy_lower_family_sapto_threshold = medicare_levy_lower_family_sapto_threshold("2024-25")) %>%
