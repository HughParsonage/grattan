...
if (FALSE) {
  change_pngs <- function(html) {
    ...
    for (i in grep("Budget2018.atlas.*\\.png", html_lines)) {
...
```{r share-of-total-personal-income-tax-by-income-decile-by-fy-year}
share201718_by_decile <- 
  s1718[, .(total = sum(new_tax)), keyby = .(Decile = ntile(Taxable_Income, 10))] %>%
  .[, "2017-18" := round(total / sum(total), 3) * 100] %>%
  .[, total := NULL] 

bound_models %>%
  .[fy_year == "2027-28"] %>%
  .[id %in% c("Budget2018_baseline", "Budget2018")] %>%
  .[, Decile := ntile(Taxable_Income, 10), keyby = "id"] %>%
  .[, .(total = sum(new_tax)), keyby = c("id", "Decile")] %>%
  .[, share := round(total / sum(total), 3) * 100, keyby = "id"] %>%
  .[]
  .[, total := NULL] %>%
  dcast(Decile ~ id, value.var = "share") %>%
  .[share201718_by_decile, on = c("Decile")]
