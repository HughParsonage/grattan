...
```{r s2425_Treasury}
if (file.exists("budget-2018_cache/s2425_Treasury.fst")) {
  s2425_Treasury <- read_fst("budget-2018_cache/s2425_Treasury.fst", 
                             as.data.table = TRUE)
} else {
  s2425_Treasury <- .project_useTreasurys[[2025L - 2016L]]
  write_fst(s2425_Treasury, "budget-2018_cache/s2425_Treasury.fst")
}
```
...
sample_files_all %>%
  ...
  rbind(avg_tax_rates_by_percentile_facet_fy[.(c("Baseline", "Budget2018", "Current")),
                                             .SD, 
                                             .SDcols = c("id", names(.))],
        use.names = TRUE,
        fill = TRUE) %>%
...
avg_tax_rate_by_percentile_brackets_cpi <-
  bound_models[.(unique(fy_year), "Baseline_brackets_cpi"),
...
```{r wt_rel1718v202425}
wt_rel1718v202425 <- 
  s2425_Treasury[, first(WEIGHT)] / s1718[, first(WEIGHT)]
```
