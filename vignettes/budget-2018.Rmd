...
alternative %>%
  .[, new_avg_tax_rate := coalesce(new_tax / Taxable_Income)] %>%
  .[, old_avg_tax_rate := coalesce(baseline_tax / Taxable_Income)] %>%
  .[, delta := new_tax - baseline_tax] %>%
...
alternative %>%
  .[, .(old_tot_tax_paid = sum(baseline_tax), 
        new_tot_tax_paid = sum(new_tax), 
        WEIGHT = sum(WEIGHT)), 
