...
```{r avg_tax_rate_by_percentile_fy_year}
sample_files_all[, tax := income_tax(Taxable_Income, fy.year = .BY[[1L]], .dots.ATO = .SD), keyby = "fy.year"][, avg_tax_rate := coalesce(tax / Taxable_Income, 0)]
sample_files_all %>%
  ...
  rbind(avg_tax_rates_by_percentile_facet_fy[id != "Current", .SD, .SDcols = c("id", names(.))],
        use.names = TRUE,
        fill = TRUE) %>%
  setkey(fy_year, Taxable_Income_percentile) %>%
  .[] %>%
  .[, avg_tax_rate_rel := avg_tax_rate - first(avg_tax_rate), keyby = .(Taxable_Income_percentile)] %>%
  .[, colour := if_else(is.na(id),
                       grey(scale10(fy2yr(fy_year))), 
  ...
  .[Taxable_Income_percentile == 100L & !is.na(id), label := paste0(id, "\n", as.character(fy_year))] %>% 
  ...
  geom_line(size = 0.75, se = FALSE) +
  # geom_blank(aes(x = 105, y = 0)) +
  # geom_label_repel(aes(label = label), nudge_x = 2, force = 0.8, na.rm = TRUE) +
  scale_y_continuous(labels = percent) +
  scale_color_identity()
```
