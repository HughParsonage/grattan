...
All instances of `model_income_tax` require `sample_file` and
`baseline_fy`. These define the `baseline_tax` column. When another
tax argument is `NULL`, the `new_tax` value is calculated using the
...
Note that the top marginal tax rate must include the temporary budget repair levy. This
causes a slight difference between the two taxes because the small business tax offset 
does not offset levies.
...
modification require changes to multiple parameters. To assist, warnings 
are emitted whenever a parameter are not internally consistent.
