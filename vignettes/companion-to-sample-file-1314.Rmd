...
tot_CG_amt <- 
  sample_file %$%
  sum(Tot_CY_CG_amt * WEIGHT)

tot_Net_CG_amt <- 
  sample_file %$%
  sum(Net_CG_amt * WEIGHT)
...
sample_files_all %>%
  ...
  mutate(label = ifelse(age_imp == max(age_imp), fy.year, NA_character_),
         ...
         Age = age_imp) %>%
         {
           ...
           ggplot(., aes(x = Age, y = as.numeric(is_CG), color = factor(fy.year), group = factor(fy.year))) + 
             scale_y_continuous(label = percent) + 
             scale_color_viridis(discrete = TRUE) + 
  ...
  mutate(label = ifelse(Age == max(Age), fy.year, NA_character_), 
         label.y = mean(mean_Net_CG[Age > 70])) %>%
         {
           ggplot(., aes(x = Age, y = mean_Net_CG, color = factor(fy.year), group = factor(fy.year))) + 
             ...
             geom_text(aes(label = label, y = label.y, size = last_fy, nudge_x = ifelse(last_fy, 1, 0)),
                       hjust = 0, 
                       vjust = 0, 
                       fontface = "bold", 
                       na.rm = TRUE) + 
