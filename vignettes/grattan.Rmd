...
```{r decode_age_range}
decode_age_range <- function(DT) {
  stopifnot("age_range" %in% names(DT))
  DT[age_range_decoder, on = "age_range"]
}
```
...
s1819_lito_1000_singles <- 
  s1819 %>% 
  .[age_range > 1L & Partner_status == 0] %>%
  model_income_tax(baseline_fy = "2018-19", 
                   lito_max_offset = 1000)
