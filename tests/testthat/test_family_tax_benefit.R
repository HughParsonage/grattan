...
test_that("Fringe cases", {
  ...
  temp <- data.table(HH_id = c(1, 1), pers_id = 1:2, age = c(35, 10), income = c(70000, 0),
                     in_secondary_school = F, single_parent = T, other_allowance_benefit_or_pension = F, maintenance_income = 0, maintenance_children = 0)
  expect_equal(family_tax_benefit(temp), 
  ...
  temp <- data.table(HH_id = c(1, 1), pers_id = 1:2, age = c(35, 18), income = c(70000, 0),
                     in_secondary_school = F, single_parent = c(T, F), other_allowance_benefit_or_pension = F, maintenance_income = 0, maintenance_children = 0)
  expect_equal(family_tax_benefit(temp), 
               data.table(HH_id = 1, ftbA_incl_supplement = 0, ftbB_incl_supplement = 0))
  #child receieving youth allowance
  temp <- data.table(HH_id = c(1, 1), pers_id = 1:2, age = c(35, 10), income = c(70000, 0),
                     in_secondary_school = F, single_parent = T, other_allowance_benefit_or_pension = c(F, T) , maintenance_income = 0, maintenance_children = 0)
  expect_equal(family_tax_benefit(temp), 
               data.table(HH_id = 1, ftbA_incl_supplement = 0, ftbB_incl_supplement = 0),
               tol=1e-02)
})
...
test_that("Maintenance", {
  #1 child
  temp <- data.table(HH_id = c(1, 1, 1), pers_id = 1:3, age = c(35, 30, 5), income = c(30000, 30000, 0),
                     in_secondary_school = F, single_parent = F, other_allowance_benefit_or_pension = F, maintenance_income = c(3000, 0, 0), maintenance_children = c(1, 0, 0))
  expect_equal(family_tax_benefit(temp), 
               data.table(HH_id = 1, ftbA_incl_supplement = 2889.95, ftbB_incl_supplement = 0),
               tol = 1e-02)
  #2 children same parent
  temp <- data.table(HH_id = c(1, 1, 1, 1), pers_id = 1:4, age = c(35, 30, 5, 0), income = c(30000, 30000, 0, 0),
                     in_secondary_school = F, single_parent = F, other_allowance_benefit_or_pension = F, maintenance_income = c(6000, 0, 0, 0), maintenance_children = c(2, 0, 0, 0))
  expect_equal(family_tax_benefit(temp), 
               data.table(HH_id = 1, ftbA_incl_supplement =  7060.6, ftbB_incl_supplement = 0),
               tol = 1e-02)
  #3 children different parents
  temp <- data.table(HH_id = c(1, 1, 1, 1, 1), pers_id = 1:5, age = c(35, 30, 5, 0, 15), income = c(30000, 30000, 0, 0, 0),
                     in_secondary_school = F, single_parent = F, other_allowance_benefit_or_pension = F, maintenance_income = c(6000, 2000, 0, 0, 0), maintenance_children = c(2, 1, 0, 0, 0))
  expect_equal(family_tax_benefit(temp), 
               data.table(HH_id = 1, ftbA_incl_supplement =  13658.08, ftbB_incl_supplement = 0),
               tol = 1e-02)
})

test_that("Multiple HH - combos of previous tests", {
  temp <- data.table(HH_id = c(1, 1, 1, 2, 2, 3, 3, 3, 3, 3), pers_id = 1:10, age = c(35, 30, 5, 35, 10, 35, 30, 5, 0, 15), income = c(50000, 50000, 0, 70000, 0, 30000, 30000, 0, 0, 0),
                     in_secondary_school = F, single_parent = c(F, F, F, T, F, F, F, F, F, F), other_allowance_benefit_or_pension = F, 
